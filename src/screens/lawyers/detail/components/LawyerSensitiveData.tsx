// src/screens/lawyers/detail/components/LawyerSensitiveData.tsx
import { ShieldAlert, Users, MessageSquare, FileText, Lock } from "lucide-react";

export const LawyerSensitiveData = () => {
  return (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-white/5 border border-white/10 rounded-2xl p-6 hover:bg-white/10 transition-all duration-300">
            <div className="flex items-center gap-4 mb-4">
              <div className="p-2 rounded-lg bg-indigo-500/10"><Users className="text-indigo-400" size={20} /></div>
              <span className="text-slate-400 font-bold text-xs uppercase tracking-widest">Clientes</span>
            </div>
        ...
        <div className="mt-8 pt-8 border-t border-white/10">
          <div className="flex items-start gap-4">
            <div className="flex -space-x-3 shrink-0">
               {[1, 2, 3].map((i) => (
                <div key={i} className="w-8 h-8 rounded-full border-2 border-slate-900 bg-slate-800 flex items-center justify-center text-[9px] font-black text-slate-400">U{i}</div>
              ))}
            </div>
            <p className="text-slate-500 text-xs font-medium leading-relaxed">
              Toda interacción entre este profesional y sus clientes se encuentra bajo cifrado de extremo a extremo. El acceso a esta información está estrictamente limitado a auditorías autorizadas.
            </p>
          </div>
        </div>
      </div>
